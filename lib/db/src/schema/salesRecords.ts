...
export const salesRecordsTable = pgTable(
  "sales_records",
  {
    ...
    skuCode: text("sku_code").references(() => skusTable.skuCode),
    externalId: text("external_id"),
    soldAt: date("sold_at").notNull(),
    quantity: numeric("quantity", { precision: 14, scale: 4 }).notNull(),
    unitPriceKrw: numeric("unit_price_krw", { precision: 14, scale: 4 }),
  ...
  (t) => [
    ...
    index("sales_records_sku_date_idx").on(t.skuCode, t.soldAt),
  ],
);
